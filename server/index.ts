...
export async function createServer(): Promise<express.Application> {
  ...
  app.all(["/api/binance/p2p", "/api/binance/p2p/*", "/api/binance-p2p/*"], async (req, res) => {
    ...
    const ua = req.headers["user-agent"] || "Mozilla/5.0 (Windows NT 10.0; Win64; x64)";
    const traceId = `${Date.now().toString(36)}${Math.random().toString(36).slice(2,8)}`;
    const deviceInfo = Buffer.from(JSON.stringify({ userAgent: ua })).toString("base64");
    ...
    return res.status(502).json({ error: "All Binance P2P endpoints failed", details: lastErr });
  });
  ...
  app.all(["/api/binance/*", "/api/binance"], async (req, res) => {
    ...
    const bodyText = req.method !== "GET" && req.method !== "HEAD" ? JSON.stringify(req.body || {}) : undefined;
    ...
    return res.status(502).json({ error: "All Binance endpoints failed", details: lastErr });
  });
