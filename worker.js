...
async function handleForexRate(reqUrl) {
  ...
  const attempts = providers.map(async (p) => {
    try {
      const r = await timeoutFetch(p.url, { headers: { Accept: 'application/json' } }, 4000);
      if (!r.ok) throw new Error(`${p.name} ${r.status}`);
      const j = await r.json();
      const rate = p.parse(j);
      if (typeof rate === 'number' && isFinite(rate) && rate > 0) return { rate, provider: p.name };
    ...
    if (typeof Promise.any === 'function') {
      ...
      res = await (async () => {
        ...
        for (const a of attempts) {
          try { const r = await a; return r; } catch (e) { errs.push(e.message); }
        }
        throw new Error(errs.join('; '));
      })();
    }

    return new Response(JSON.stringify({ base, symbols: [first], rates: { [first]: res.rate }, provider: res.provider }), {
      headers: { 'content-type': 'application/json', ...corsHeaders() }
    });
  } catch (e) {
    return new Response(JSON.stringify({ error: 'Failed to fetch forex rate', details: e.message || String(e) }), {
      status: 502,
      headers: { 'content-type': 'application/json', ...corsHeaders() }
    });
  }
...
async function handleWalletBalance(reqUrl, env) {
  const publicKey = reqUrl.searchParams.get('publicKey') || reqUrl.searchParams.get('wallet') || reqUrl.searchParams.get('address');
  if (!publicKey) {
    return new Response(JSON.stringify({ error: 'Missing publicKey parameter' }), { status: 400, headers: { 'content-type': 'application/json', ...corsHeaders() } });
  }
  ...
  if (env && env.HELIUS_API_KEY) rpcCandidates.push(`https://mainnet.helius-rpc.com/?api-key=${env.HELIUS_API_KEY}`);
  ...
  for (const endpoint of rpcCandidates) {
    try {
      const res = await timeoutFetch(endpoint, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }, 6000);
      if (!res.ok) {
      ...
      const lam = typeof balLamports === 'object' && 'value' in balLamports ? balLamports.value : balLamports;
      const balanceLamports = Number(lam) ?? 0;
      const balance = balanceLamports / 1_000_000_000;
      return new Response(JSON.stringify({ publicKey, balance, balanceLamports }), { headers: { 'content-type': 'application/json', ...corsHeaders() } });
    } catch (e) {
      lastErr = e;
      continue;
    }
  ...
  return new Response(JSON.stringify({ error: lastErr ? lastErr.message : 'All RPCs failed' }), { status: 502, headers: { 'content-type': 'application/json', ...corsHeaders() } });
}

async function handleDexTokens(reqUrl) {
  const mints = reqUrl.searchParams.get('mints');
  if (!mints) return new Response(JSON.stringify({ error: "Missing 'mints' parameter" }), { status: 400, headers: { 'content-type': 'application/json', ...corsHeaders() } });
  const raw = mints.split(',').map(s => s.trim()).filter(Boolean);
  const unique = Array.from(new Set(raw));
  if (unique.length === 0) return new Response(JSON.stringify({ error: 'No valid mints provided' }), { status: 400, headers: { 'content-type': 'application/json', ...corsHeaders() } });

  const batches = [];
  for (let i=0;i<unique.length;i+=MAX_DEX_BATCH) batches.push(unique.slice(i,i+MAX_DEX_BATCH));
  ...
  const pairs = Array.from(dedup.values()).filter(p => p.chainId === 'solana');

  return new Response(JSON.stringify({ schemaVersion, pairs }), { headers: { 'content-type': 'application/json', ...corsHeaders() } });
}

async function handleDexSearch(reqUrl) {
  const q = reqUrl.searchParams.get('q');
  if (!q) return new Response(JSON.stringify({ error: "Missing 'q' parameter" }), { status: 400, headers: { 'content-type': 'application/json', ...corsHeaders() } });
  try {
    const data = await tryDexscreener(`/search/?q=${encodeURIComponent(q)}`);
    const pairs = (data?.pairs || []).filter(p => p.chainId === 'solana').slice(0,20);
    return new Response(JSON.stringify({ schemaVersion: data?.schemaVersion || '1.0.0', pairs }), { headers: { 'content-type': 'application/json', ...corsHeaders() } });
  } catch (e) {
    return new Response(JSON.stringify({ error: 'Search failed', details: e.message || String(e) }), { status: 502, headers: { 'content-type': 'application/json', ...corsHeaders() } });
  }
}

async function handleDexPrice(reqUrl) {
  const token = reqUrl.searchParams.get('tokenAddress') || reqUrl.searchParams.get('token') || reqUrl.searchParams.get('mint');
  if (!token) return new Response(JSON.stringify({ error: "Missing tokenAddress parameter" }), { status: 400, headers: { 'content-type': 'application/json', ...corsHeaders() } });
  try {
    const data = await tryDexscreener(`/tokens/${encodeURIComponent(token)}`);
    const price = data?.pairs?.[0]?.priceUsd ?? null;
    if (price) return new Response(JSON.stringify({ token, priceUsd: Number(price) }), { headers: { 'content-type': 'application/json', ...corsHeaders() } });
    return new Response(JSON.stringify({ token, priceUsd: null, message: 'Price not available' }), { status: 404, headers: { 'content-type': 'application/json', ...corsHeaders() } });
  } catch (e) {
    return new Response(JSON.stringify({ error: 'DexScreener fetch failed', details: e.message || String(e) }), { status: 502, headers: { 'content-type': 'application/json', ...corsHeaders() } });
  }
}
...
async function handleSolanaRpc(req, env) {
  ...
  if (env && env.HELIUS_API_KEY) candidates.push(`https://mainnet.helius-rpc.com/?api-key=${env.HELIUS_API_KEY}`);
  candidates.push(...DEFAULT_RPC_FALLBACKS);

  for (const endpoint of candidates) {
    try {
      const r = await timeoutFetch(endpoint, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }, 7000);
      const text = await r.text();
      return new Response(text, { status: r.status, headers: { 'content-type': r.headers.get('content-type') || 'application/json', ...corsHeaders() } });
    } catch (e) {
      // try next
    }
  }
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (pathname === 'api/wallet/balance' || pathname === 'api/balance') return handleWalletBalance(url, env);
      ...
      if (pathname.startsWith('api')) {
        return new Response(JSON.stringify({ error: 'API endpoint not found', path: `/${pathname}` }), { status: 404, headers: { 'content-type': 'application/json', ...corsHeaders() } });
      }
      ...
      return new Response(JSON.stringify({ error: 'Unhandled error', details: (err && err.message) || String(err) }), { status: 500, headers: { 'content-type': 'application/json', ...corsHeaders() } });
    }
  }
};
