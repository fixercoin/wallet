/**
 * Backendless KV Store Adapter for Cloudflare Functions
 * Provides KV-like interface using Backendless REST API for Functions environment
 */
...
/**
 * BackendlessKVStore provides the same interface as KVStore but uses Backendless API
 */
export class BackendlessKVStore {
  ...
  private getTableForKey(key: string): string {
    const prefix = key.split(":")[0];

    const tableMap: Record<string, string> = {
      stakes: BACKENDLESS_TABLES.ORDERS,
      rewards: BACKENDLESS_TABLES.ORDERS,
      orders: BACKENDLESS_TABLES.ORDERS,
      payment_methods: BACKENDLESS_TABLES.PAYMENT_METHODS,
      notifications: BACKENDLESS_TABLES.NOTIFICATIONS,
      escrow: BACKENDLESS_TABLES.ESCROW,
      dispute: BACKENDLESS_TABLES.DISPUTES,
      p2p_matched: BACKENDLESS_TABLES.MATCHES,
      p2p: BACKENDLESS_TABLES.ROOMS,
      p2p_merchant_stats: BACKENDLESS_TABLES.MERCHANT_STATS,
    };

    return tableMap[prefix] || BACKENDLESS_TABLES.ORDERS;
  }

  private sanitizeRecordId(key: string): string {
    return key.replace(/:/g, "_").substring(0, 255);
  }

  async get(key: string): Promise<string | null> {
    try {
      const table = this.getTableForKey(key);
      const recordId = this.sanitizeRecordId(key);
  ...
  async list(options?: {
    ...
    try {
      const table = this.getTableForKey(options?.prefix || "orders");
