  ...
  async fetch(req, env, ctx) {
    ...
    if (pathname === "/api/swap" && req.method === "POST") {
      try {
          ...
          try {
            ...
            if (resp.ok) {
              ...
              if (provider === "pumpfun") {
                ...
                return json(
                  {
                    error: `Pump.fun swap failed ${resp.status}`,
                    details: errorText,
                  },
                  { status: resp.status, headers: corsHeaders },
                );
        ...
        if (inputMint && outputMint)
          helpText +=
            "For Meteora swaps, provide inputMint, outputMint, amount, wallet.";
        if (inputMint)
          helpText += " For Jupiter, also provide routePlan from quote.";
    ...
    if (pathname === "/api/swap/build" && req.method === "POST") {
      try {
        ...
        const swapTx =
          (data && data.swap && data.swap.swapTransaction) ||
          (data && (data.swapTransaction || data.swapTransactionBase64));
        if (swapTx)
          return json(
            {
              swapTransaction: swapTx,
              signed: false,
              source: (data && (data._source || data.source)) || null,
            },
            { headers: corsHeaders },
          );
        return json(
    ...
    if (pathname === "/api/swap/submit" && req.method === "POST") {
      try {
        const body = await parseJSON(req);
        const signedTx =
          body && (body.signedTx || body.tx || body.signedTransaction);
        if (!signedTx)
          return json(
        ...
        try {
          ...
          const parsed = JSON.parse(
            String((rpcResult && rpcResult.body) || "{}"),
          );
          ...
          return json(
            {
              ...
              details:
                rpcErr && rpcErr.message ? rpcErr.message : String(rpcErr),
            },
            { status: 502, headers: corsHeaders },
          );
        }
