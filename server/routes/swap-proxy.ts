...
const WORKER_BASE = process.env.VITE_API_BASE_URL || "https://fixorium-proxy.khanbabusargodha.workers.dev";
...
async function forwardRequest(path: string, req: any) {
  ...
  try {
    ...
    if (contentType.includes('application/json')) {
      try { return { status: resp.status, body: JSON.parse(text) }; } catch (e) { return { status: resp.status, body: text }; }
    }
...
export const handleSwapProxy: RequestHandler = async (req, res) => {
  try {
    ...
    return res.status(502).json({ error: 'Failed to proxy swap request', details: e?.message || String(e) });
  }
};
...
export const handleQuoteProxy: RequestHandler = async (req, res) => {
  try {
    ...
    return res.status(502).json({ error: 'Failed to proxy quote request', details: e?.message || String(e) });
  }
};
...
export const handleMeteoraQuoteProxy: RequestHandler = async (req, res) => {
  try {
    ...
    return res.status(502).json({ error: 'Failed to proxy meteora quote', details: e?.message || String(e) });
  }
};
...
export const handleMeteoraSwapProxy: RequestHandler = async (req, res) => {
  try {
    ...
    return res.status(502).json({ error: 'Failed to proxy meteora swap', details: e?.message || String(e) });
  }
};
...
export const handleSolanaSendProxy: RequestHandler = async (req, res) => {
  try {
    ...
    return res.status(502).json({ error: 'Failed to proxy solana-send', details: e?.message || String(e) });
  }
};
...
export const handleSolanaSimulateProxy: RequestHandler = async (req, res) => {
  try {
    ...
    return res.status(502).json({ error: 'Failed to proxy solana-simulate', details: e?.message || String(e) });
  }
};
