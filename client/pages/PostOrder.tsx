      ...
      <div className="max-w-2xl mx-auto px-4 py-6">
        <div className="wallet-card rounded-2xl p-4">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="text-sm text-gray-500">Mode: {mode.toUpperCase()}</div>
          </div>

          <div className="space-y-3">
            <div>
              <label className="block text-xs text-gray-500 mb-1">Select token</label>
              <select value={token} onChange={(e) => setToken(e.target.value)} className="w-full border rounded-xl px-3 py-2 bg-white">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input type="number" value={minAmount} onChange={(e) => setMinAmount(e.target.value)} className="w-full border rounded-xl px-3 py-2 bg-white" />
              </div>
              <div>
                <label className="block text-xs text-gray-500 mb-1">Maximum amount (PKR)</label>
                <input type="number" value={maxAmount} onChange={(e) => setMaxAmount(e.target.value)} className="w-full border rounded-xl px-3 py-2 bg-white" />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input type="number" value={price} onChange={(e) => setPrice(e.target.value)} className="flex-1 border rounded-xl px-3 py-2 bg-white" />
                <button type="button" onClick={() => autoFillPrice(token)} className="px-3 py-2 bg-gray-200 rounded-xl">Auto</button>
              </div>
            </div>
            ...
            <div className="flex gap-2">
              <button onClick={clearForm} className="flex-1 bg-gray-200 rounded-xl px-3 py-2">Clear</button>
              <button onClick={handleCreate} className="flex-1 bg-gradient-to-r from-purple-500 to-blue-600 text-white rounded-xl px-3 py-2">Create Order</button>
            </div>

            <div className="text-xs text-gray-500">Tip: switch between Buy and Sell above. You can auto-fill price based on the selected token.</div>
          </div>
        </div>
