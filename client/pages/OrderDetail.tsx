...
export default function OrderDetail() {
  ...
  useEffect(() => {
    try {
      ...
      const foundP = Array.isArray(p) ? p.find((o: any) => String(o.id) === String(orderId)) : null;
      const foundC = Array.isArray(c) ? c.find((o: any) => String(o.id) === String(orderId)) : null;
      if (foundC) {
        setOrder(foundC);
        setStatus("completed");
      } else if (foundP) {
            ...
            <CardContent className="space-y-4 p-4">
              ...
              {typeof order.amountPKR !== "undefined" && (
                <div className="flex items-center justify-between">
                  ...
                  <div className="font-semibold">{Number(order.amountPKR).toLocaleString()}</div>
                </div>
              )}
              {typeof order.amountTokens !== "undefined" && (
                <div className="flex items-center justify-between">
                  <div className="text-xs opacity-80">Amount Tokens</div>
                  <div className="font-semibold">{Number(order.amountTokens).toFixed(6)} {order.token}</div>
                </div>
              )}
              {typeof order.pricePKRPerQuote !== "undefined" && (
                <div className="flex items-center justify-between">
                  <div className="text-xs opacity-80">Exchange Rate</div>
                  <div className="font-semibold">1 {order.token} = {order.pricePKRPerQuote < 1 ? Number(order.pricePKRPerQuote).toFixed(6) : Number(order.pricePKRPerQuote).toFixed(2)} PKR</div>
                </div>
              )}
              {order.paymentMethod && (
                <div className="flex items-center justify-between">
