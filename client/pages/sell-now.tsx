...
export default function SellNow() {
  ...
  const selectedTokenBalance = useMemo(() => {
    const t = (walletTokens || []).find((tk) => (tk.symbol || "").toUpperCase() === selectedToken.symbol.toUpperCase());
    return t?.balance || 0;
    ...
    const fetchTokens = async () => {
      try {
        ...
        const enriched = DEFAULT_TOKENS.map((token) => {
          ...
          return { ...token, logo: dexData?.info?.imageUrl || token.logo, price: dexData?.priceUsd ? parseFloat(dexData.priceUsd) : undefined } as TokenOption;
        });
        setTokens(enriched);
        ...
        <Card className="bg-transparent backdrop-blur-xl rounded-md">
          <CardContent className="space-y-6 pt-6">
            ...
            <div className="p-3 rounded-lg bg-[#1a2540]/50 border border-[#FF7A5C]/30 text-white">
              ...
              <div className="mt-1 text-sm"><span className="font-semibold">{selectedTokenBalance.toFixed(6)} {selectedToken.symbol}</span></div>
            </div>

            <div>
              <label className="block font-medium text-white/80 mb-2">Amount ({selectedToken.symbol})</label>
              <input type="number" value={sellAmountTokens} onChange={(e) => setSellAmountTokens(e.target.value)} placeholder={`Enter amount in ${selectedToken.symbol}`} className="w-full px-4 py-3 rounded-lg bg-[#1a2540]/50 border border-[#FF7A5C]/30 focus:outline-none focus:ring-2 focus:ring-[#FF7A5C] text-white placeholder-white/40" min="0" step="0.000001" />
            </div>
            ...
            <div className="p-4 rounded-lg bg-[#1a2540]/50 border border-[#FF7A5C]/30">
              <div className="space-y-3">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-bold text-[#FF7A5C]">{(Number(sellAmountTokens || 0) * (exchangeRate || 0)).toFixed(2)} PKR</span>
                </div>
              </div>
            </div>

            <Button onClick={handleSellClick} disabled={loading || !sellAmountTokens || Number(sellAmountTokens) <= 0 || !exchangeRate} className="w-full h-12 rounded-lg font-semibold transition-all duration-200 bg-gradient-to-r from-[#FF7A5C] to-[#FF5A8C] hover:from-[#FF6B4D] hover:to-[#FF4D7D] text-white shadow-lg hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed">
              {loading ? (<><Loader2 className="w-5 h-5 mr-2 animate-spin" />Processing...</>) : ("SELL FOR PKR")}
            </Button>
          </CardContent>
        </Card>
      </div>
