  ...
  try {
    ...
    if ((provider === 'meteora' || provider === 'auto') && inputMint && outputMint && amount) {
      ...
      try {
        ...
        if (!resp.ok) {
          return res.status(resp.status).json({ error: 'Meteora swap failed', details: jsonBody });
        }
        return res.json({ source: 'meteora', swap: jsonBody, signingRequired: true });
      } catch (e: any) {
        clearTimeout(timeout);
    ...
    return res.status(400).json({ error: 'Unsupported swap request or missing fields (inputMint, outputMint, amount)' });
  } catch (e: any) {
    return res.status(502).json({ error: 'Failed to process swap locally', details: e?.message || String(e) });
  }
};
