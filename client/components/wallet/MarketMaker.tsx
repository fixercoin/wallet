...
import { ArrowLeft, AlertTriangle, Zap, TrendingUp } from "lucide-react";
...
const TOKEN_ACCOUNT_RENT = 0.002; // SOL
    ...
    return (
      <div className="w-full max-w-2xl mx-auto px-4 py-8">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h1 className="text-2xl font-bold">Market Maker Session</h1>
        </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <Label className="text-sm text-gray-600">Number of Makers</Label>
                <p className="text-lg font-bold">
                  {currentSession.numberOfMakers}
                </p>
              </div>
              <div>
                <Label className="text-sm text-gray-600">Order Range (SOL)</Label>
                <p className="text-sm">
              ...
              <div>
                <Label className="text-sm text-gray-600">Sell Strategy</Label>
                <p className="text-sm capitalize">{currentSession.sellStrategy}</p>
              </div>
              <div>
                <Label className="text-sm text-gray-600">Estimated Fees</Label>
                ...
                {currentSession.makers.map((maker) => (
                  ...
                  >
                    <div className="flex justify-between">
                      <span className="font-mono">{maker.id}</span>
                      <span
            ...
            <div className="border-t pt-4 flex gap-2">
              {currentSession.status === "setup" && (
                <Button
                  ...
                  className="flex-1"
                >
                  {isLoading ? "Starting..." : "Start Market Making"}
                </Button>
              )}
              {currentSession.status === "running" && (
                <div className="flex-1 py-2 px-4 bg-green-50 border border-green-200 rounded text-sm text-green-800 font-medium flex items-center gap-2">
  ...
  return (
    <div className="w-full max-w-2xl mx-auto px-4 py-8">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-2xl font-bold">FIXORIUM Market Maker</h1>
      </div>
      ...
      <div className="space-y-6">
        <Card>
          ...
          <CardContent className="space-y-6">
            ...
            <div>
              <Label htmlFor="num-makers">Number of Makers</Label>
              <Input
                id="num-makers"
                type="number"
                min="1"
            ...
            <div>
              <Label className="mb-2 block">Order Amount (SOL)</Label>
            ...
            <div>
              <Label className="mb-2 block">
                Delay Between Buy Transactions (seconds)
              </Label>
              <div className="space-y-2">
                <div>
                  <Label className="text-xs text-gray-600">Minimum Delay</Label>
                  <Input
                    type="number"
                    min="0"
                ...
                <div>
                  <Label className="text-xs text-gray-600">Maximum Delay</Label>
                  <Input
                    type="number"
                    min="0"
            ...
            <div>
              <Label htmlFor="sell-strategy">
                What would you like to do with purchased tokens?
              </Label>
              <Select value={sellStrategy} onValueChange={(value: any) => setSellStrategy(value)}>
                <SelectTrigger id="sell-strategy">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
            ...
            {sellStrategy === "manual-target" && (
              <div>
                <Label htmlFor="price-target">Manual Price Target</Label>
                <Input
                  id="price-target"
                  type="number"
                  step="0.00001"
            ...
            {sellStrategy === "gradually" && (
              <div>
                <Label htmlFor="gradual-sell">Gradually Sell (%)</Label>
                <Input
                  id="gradual-sell"
                  type="number"
                  min="0.1"
            ...
            <div className="border-t pt-4">
              <div className="space-y-2 bg-blue-50 p-4 rounded border border-blue-200">
                <div className="flex justify-between text-sm">
                  <span className="text-gray-700">Total SOL Needed:</span>
                  <span className="font-bold">◎ {totalSOLNeeded.toFixed(4)}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-700">Estimated Total Fees (1%):</span>
                  <span className="font-bold">◎ {totalFees.toFixed(4)}</span>
                </div>
                <div className="flex justify-between text-sm pt-2 border-t border-blue-200">
                  <span className="text-gray-700">Your SOL Balance:</span>
                  <span
                    className={`font-bold ${canAfford ? "text-green-600" : "text-red-600"}`}
                  >
                ...
                {!canAfford && (
                  <div className="text-xs text-red-600 mt-2">
                    You need {(totalSOLNeeded - solBalance).toFixed(4)} more SOL
                  </div>
                )}
              </div>
            </div>
            ...
            <Button
              ...
              size="lg"
            >
              {isLoading ? "Setting up..." : "Create Market Maker Bot"}
            </Button>
          </CardContent>
        </Card>
