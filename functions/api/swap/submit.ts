export const onRequest: PagesFunction = async ({ request, env }) => {
  ...
  if (!txBase64 || typeof txBase64 !== "string") {
    return Response.json(
      { error: "Missing signed transaction (base64)", expected: ["signedTx", "signedTransaction", "signedBase64", "tx"] },
      { status: 400 },
    );
  }
  ...
  return Response.json(
    { error: "rpc_send_failed", details: lastErr || "All RPC endpoints failed" },
    { status: 502 },
  );
};
