...
async function tryFetchJson(url: string, opts: any = {}) {
  ...
  try {
    ...
    try {
      return { ok: resp.ok, status: resp.status, json: text ? JSON.parse(text) : null, text };
    } catch (e) {
...
export const handleLocalQuote: RequestHandler = async (req, res) => {
  try {
    ...
    const outputMint = String(req.query.outputMint || req.query.output_mint || "");
    ...
    if (inputMint && outputMint && amount) {
      try {
        ...
        const r = await tryFetchJson(url.toString(), { headers: { Accept: "application/json" } });
        if (r.ok && r.json) return res.json({ source: "meteora", quote: r.json });
      } catch (e) {
    ...
    if (inputMint && outputMint && amount) {
      try {
        ...
        const r = await tryFetchJson(url, { headers: { Accept: "application/json" } });
        if (r.ok && r.json) return res.json({ source: "jupiter", quote: r.json });
      } catch (e) {}
    }
    ...
    if (inputMint || mint) {
      ...
      try {
        ...
        if (r.ok && r.json) {
          ...
          if (firstPair) {
            return res.json({ source: "dexscreener", quote: { inAmount: amount || "1", outAmount: firstPair.priceUsd ?? 0, priceImpact: 0 } });
          }
        }
      } catch (e) {}
    }

    return res.status(502).json({ error: "Failed to fetch quote from providers" });
  } catch (e: any) {
    return res.status(502).json({ error: "Quote handler failed", details: e?.message || String(e) });
  }
};
